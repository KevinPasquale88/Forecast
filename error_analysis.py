...
import numpy as np
import pandas as pd
from function import get_output_dirs, models_all, num_cols, num_cols_diabetes130, plot_error_rates, plot_feature_deviation
...
def analyze_errors(dataset="heart_disease"):
    ...
    X_raw = pd.read_csv(os.path.join(dirs["preprocessing"], "X_train_raw.csv"))
    n_records = len(X_raw)
    ...
    for model in models_all:
        ...
        y_true = np.load(os.path.join(dirs["results"], f"{name}_y_true.npy"))
        y_pred = np.load(os.path.join(dirs["results"], f"{name}_y_pred.npy"))
        y_score = np.load(os.path.join(dirs["results"], f"{name}_y_score.npy"))
        val_idx = np.load(os.path.join(dirs["results"], f"{name}_val_idx.npy"))
    ...
    df_summary.to_csv(os.path.join(dirs["results"], "error_summary.csv"), index=False)
    plot_error_rates(df_summary, results_dir=dirs["results"])
    ...
    df_hardest.to_csv(os.path.join(dirs["results"], "hardest_cases.csv"), index=False)
    ...
    df_deviation.to_csv(os.path.join(dirs["results"], "feature_deviation.csv"), index=False)
    plot_feature_deviation(df_deviation, results_dir=dirs["results"])

    print("\n=== ERROR ANALYSIS COMPLETED ===")
    print(df_summary.to_string(index=False))
    print(f"Hardest cases saved: {os.path.join(dirs['results'], 'hardest_cases.csv')} ({len(df_hardest)} records)")
    print(f"Feature deviation saved: {os.path.join(dirs['results'], 'feature_deviation.csv')}")
